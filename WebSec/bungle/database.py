...
def connect():
    ...
    return mdb.connect(host='localhost', user='yuankai4', password='423510c156a3f2680ee62f3fff42ca1c05fa87b05551f18fa5130b5dfe1017cc', db='project2')
...
def createUser(username, password):
    ...
    insert_user = "INSERT INTO users(username, password, passwordhash) VALUES(%s, %s, %s)"
...
def addHistory(user_id, query):
    ...
    insert_history = "INSERT INTO history(user_id, query) VALUES(%s, %s)"
    data = (user_id, query)
...
def getHistory(user_id):
    ...
    select_history = "SELECT query FROM history WHERE user_id = %s ORDER BY id desc LIMIT 15"
    data = (user_id)
    cur.execute(select_history, data)
    rows = cur.fetchall()
